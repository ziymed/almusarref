...
#الفعل الثلاثي المثال
		...
		if (verb_nm.startswith(ALEF_HAMZA_ABOVE) or verb_nm.startswith(HAMZA)) and root[0]==ALEF_HAMZA_ABOVE:
...
##		verb_nm=ar_strip_marks_keepshadda(verb);
##		if len(verb_nm)==3 and verb_nm.endswith(WAW) and marks[1]==FATHA:
####		if len(letters)==3 and letters.endswith(WAW):
##		    verb_nm=verb_nm[:-1]+YEH;
##		primal_stem=verb;
		first_mark=marks[0];
		# treat first_mark
## حالة فاعل فوعل
...
#---------------------------------------------------------------------
	...
	def conjugate_tense_pronoun(self,tense,pronoun):
		prefix=TableTenseProunoun[tense][pronoun][0];
		suffix=TableTenseProunoun[tense][pronoun][1];
